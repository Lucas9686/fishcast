...
- `matchesWaterTypeFilter(fish, saltwater, freshwater)` — Implementation:
  1. If neither `saltwater` nor `freshwater` is true, return true (no water filter active).
  2. If fish.waterTypes is missing or not an Array, return false (defensive guard).
  3. If `saltwater` is true, check if fish.waterTypes contains any value from SALTWATER_TYPES (using `.some()` + `.indexOf()`).
  4. If `freshwater` is true, check if fish.waterTypes contains any value from FRESHWATER_TYPES (same approach).
  5. If BOTH `saltwater` AND `freshwater` are true, return true if fish matches EITHER group (OR logic): `return matchesSalt || matchesFresh`.
  6. If only one is true, return whether fish matches that single group.
  Example skeleton:
  ```javascript
...
**Event listener wiring for each filter button:**

Store references to all toggle buttons so the clear-all handler can remove their active classes. Wire each button with a click listener using this pattern:

```javascript
// Store button references for clear-all
var filterButtons = [];

// For each toggle button (freshwaterBtn, saltwaterBtn, seasonBtn, edibilityBtn):
...
resetBtn.addEventListener('click', function() {
    ...
    // Also reset category buttons to 'alle' active state
    applyFilter();
});
```
